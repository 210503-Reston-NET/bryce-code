...
namespace RRWebUI.Controllers
{
    public class RestaurantController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(RestaurantVM restaurantVM, IFormCollection collection)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    Restaurant editRestaurant = _restaurantBL.GetRestaurantById(restaurantVM.Id);
                    editRestaurant.Name = restaurantVM.Name;
                    editRestaurant.City = restaurantVM.City;
                    editRestaurant.State = restaurantVM.State;
                    _restaurantBL.EditRestaurant(editRestaurant);
                    return RedirectToAction(nameof(Index));
                }
                return View();
            }
